...
export default function PlayerPage({params}){
  const {slug}=params; const [player,setPlayer]=useState(null); const [error,setError]=useState(null);
  useEffect(()=>{(async()=>{try{const r=await fetch(`/api/players/${slug}`);const d=await r.json();if(!r.ok||!d.ok)throw new Error();setPlayer(d.player);}catch{setError("Игрок не найден или KV не настроен")}})();},[slug]);
  ...
  return (<main className="min-h-screen bg-bgdeep text-white container py-10">
    <div className="flex items-center gap-4 mb-6">
      ...
      <div>
        <h1 className="text-3л font-extrabold">{player.nickname}</h1>
    ...
    <section className="grid md:grid-cols-2 gap-4">
      <div className="bg-panel rounded-2xl p-6">
        ...
        <p className="text-gray-400">Матчи, победы и т.д. (подключим позже).</p>
      </div>
      <div className="bg-panel rounded-2xl p-6">
        <h3 className="text-xl font-semibold mb-2">О себе</h3>
        <p className="text-gray-400">Короткая био игрока — можно добавить в форму.</p>
      </div>
    </section>
  </main>);
}