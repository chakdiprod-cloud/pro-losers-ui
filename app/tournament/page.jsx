...
const tabs = ["Обзор", "Сетка", "Матчи", "Игроки", "Правила"];

export default function TournamentPage() {
  const [active, setActive] = useState("Обзор");

  return (
    <main className="min-h-screen bg-bgdeep text-white px-6 py-10 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <Link href="/" className="text-gray-300 hover:text-white">&larr; На главную</Link>
        <Button className="bg-primary hover:bg-accent relative overflow-hidden">
          Участвовать
      ...
      {active === "Сетка" && (
        <section className="bg-panel rounded-2xl p-6 overflow-x-auto">
          ...
          <div className="min-w-[900px] grid grid-cols-4 gap-6">
            {[["1/8",4],["1/4",2],["1/2",1],["Финал",1]].map(([title, pairs], col)=> (
              <div key={title}>
                <div className="text-gray-400 mb-2">{title}</div>
                {[...Array(pairs)].map((_,i)=>(
                  <div key={i} className="mb-4 p-3 rounded-xl bg-[#2a2b2f]">
                    <div>Игрок A</div>
                    <div>Игрок B</div>
                  </div>
                ))}
